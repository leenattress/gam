...
class Stage {
  ...
  constructor({ target, width, height, refreshRate, update, init, onMouseClick, onMouseMove }) {
    ...
    this.rgbpal = this.hexpal.map((hex) => {
      return this.hexToGlRgb(hex);
    });


    this.pixels = this.canvas.width * this.canvas.height;
    this.framebuffer = new Int32Array(this.pixels);
    ...
    for (var i = 0; i < this.canvas.width; i++) {
      for (var j = 0; j < this.canvas.height; j++) {
        ...
        var k = index * 5;
        this.framebufferInternal[k + 0] = i; // x
        this.framebufferInternal[k + 1] = j; // y
  ...
  clearViewport() {
    //this.framebuffer = JSON.parse(JSON.stringify(this.framebufferBlank));

    this.framebuffer = this.framebuffer = new Int32Array(this.pixels).fill(0);
  ...
  pset(colour, x, y) {
    const memLoc = this.xyToMemory(x, y);
  ...
  rectFill(x1, y1, width, height, col) {
  ...
  framebufferToScreen() {
    ...
    for (var i = 0; i < this.canvas.width; i++) {
      for (var j = 0; j < this.canvas.height; j++) {
        //colour = this.fastInt(Math.random() * 31);
        colour = this.framebuffer[index];
    ...
    this.gl.uniform2f(this.program.uResolution, this.gl.canvas.width, this.gl.canvas.height);
...
class Sprites {
  ...
  add(name, path) {
    this.sprites[name] = this.loadPng(path);
